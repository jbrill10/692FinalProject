...
from gpt4VideoAnalyzer import get_ai_feedback
...
def main(cfg):
    ...
    # Load the task code
    task = cfg.env.task
    task_description = "to make the humanoid reach the end as fast as possible while running like a human would. The legs alternate between a stance phase and a swing phase. During the stance phase, one foot is in contact with the ground, providing support and propulsion. The knee is generally slightly bent upon initial contact, then extends during the support phase, and flexes again to prepare for the swing phase. The swing phase involves the leg that is not in contact with the ground swinging forward to prepare for its stance phase. This swinging leg moves through hip flexion followed by knee flexion to advance the leg with minimal effort and aerodynamic resistance. The arms are bent at the elbows and move in a reciprocal motion with the legs. As one leg moves forward during its swing phase, the opposite arm swings forward, and vice versa. This arm movement helps to balance the body and maintain momentum.\n- The torso remains upright with a slight forward lean. This position helps to maintain balance and facilitate forward motion. The head is held up, generally looking forward to anticipate terrain and maintain balance.\n\nThe movement is rhythmic, and each cycle of leg and arm movement constitutes one stride."
    suffix = cfg.suffix
    model = cfg.model
    ...
    for iter in range(cfg.iteration):
        ...
        for response_id in range(cfg.sample):
            ...
            for retry in range(cfg.max_retries): # loop for retrying failed runs
                ...
                if run_success:
                    ...
                    content += f"\nMost importantly, please alter the reward function to address this crucial feedback: {get_ai_feedback()}\n" 
        ...
        logging.info("REVIEWING CONTENTS...")
        # Select the best code sample based on the success rate
        best_sample_idx = np.argmax(np.array(successes))
        best_content = contents[best_sample_idx]
        logging.info(best_content)
